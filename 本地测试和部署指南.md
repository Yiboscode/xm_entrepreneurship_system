# 本地测试和服务器部署指南

## 📋 目录
1. [本地测试流程](#本地测试流程)
2. [服务器部署流程](#服务器部署流程)
3. [Docker容器管理](#docker容器管理)
4. [日志查看和调试](#日志查看和调试)
5. [常见问题](#常见问题)

---

## 一、本地测试流程

### ✅ 优点
- 快速调试，修改即时生效
- 可以使用IDEA断点调试
- 前端热更新，开发效率高
- 连接真实的服务器数据库

### 🔧 步骤

#### 1. 确认MySQL已在服务器部署
```bash
# 在服务器上检查
docker ps | grep mysql
```

#### 2. 启动后端（IDEA中）
- 打开 `SpringbootApplication.java`
- 点击运行按钮或 `Shift+F10`
- 或者使用Maven命令：
```bash
cd springboot
mvn spring-boot:run
```

#### 3. 启动前端
打开新终端：
```bash
cd vue
npm install  # 第一次需要安装依赖
npm run dev
```

#### 4. 访问测试
打开浏览器访问：http://localhost:5173

#### 5. 测试清单
- [ ] 管理员登录（admin/admin）
- [ ] 用户注册和登录
- [ ] 教师注册和认证
- [ ] 创建选题
- [ ] 提交选题评价
- [ ] 文件上传下载
- [ ] 项目管理
- [ ] 团队管理

---

## 二、服务器部署流程

### 方式A：完整Docker部署（推荐生产环境）

#### 1. 打包应用
```bash
# 打包后端
cd springboot
mvn clean package -DskipTests

# 打包前端
cd vue
npm run build
```

#### 2. 上传到服务器
使用SFTP工具或命令：
```bash
# 上传整个项目目录
scp -r springboot ubuntu@collegetopics.cn:~/xm_entrepreneurship_system/
scp -r vue ubuntu@collegetopics.cn:~/xm_entrepreneurship_system/
scp deploy/docker-compose.yml ubuntu@collegetopics.cn:~/xm_entrepreneurship_system/
```

#### 3. 构建并启动容器
在服务器上执行：
```bash
cd ~/xm_entrepreneurship_system

# 构建镜像
docker build -t entrepreneurship-backend ./springboot
docker build -t entrepreneurship-frontend ./vue

# 启动后端
docker run -d \
  --name entrepreneurship_backend \
  --restart always \
  -p 9090:9090 \
  -v $(pwd)/files:/app/files \
  --link entrepreneurship_mysql:mysql \
  -e SPRING_DATASOURCE_URL=jdbc:mysql://mysql:3306/entrepreneurship_system?useUnicode=true&characterEncoding=utf-8&allowMultiQueries=true&useSSL=false&serverTimezone=GMT%2b8&allowPublicKeyRetrieval=true \
  -e SPRING_DATASOURCE_PASSWORD=root123456 \
  entrepreneurship-backend

# 启动前端
docker run -d \
  --name entrepreneurship_frontend \
  --restart always \
  -p 80:80 \
  --link entrepreneurship_backend:backend \
  entrepreneurship-frontend
```

#### 4. 验证部署
```bash
# 查看容器状态
docker ps

# 查看日志
docker logs entrepreneurship_backend
docker logs entrepreneurship_frontend

# 访问测试
curl http://localhost:9090/
curl http://localhost/
```

### 方式B：后台直接运行（适合调试）

#### 1. 上传jar包
```bash
# 本地打包
cd springboot
mvn clean package -DskipTests

# 上传到服务器
scp target/springboot-0.0.1-SNAPSHOT.jar ubuntu@collegetopics.cn:~/xm_entrepreneurship_system/
```

#### 2. 使用nohup后台运行
在服务器上执行：
```bash
cd ~/xm_entrepreneurship_system

# 后台运行，日志输出到文件
nohup java -jar springboot-0.0.1-SNAPSHOT.jar > app.log 2>&1 &

# 查看进程
ps aux | grep java

# 查看日志（实时）
tail -f app.log

# 停止应用
# 找到进程ID
ps aux | grep springboot
# 杀死进程
kill <PID>
```

#### 3. 使用screen保持会话（推荐）
```bash
# 安装screen
sudo apt install screen

# 创建新会话
screen -S backend

# 在screen中启动应用
cd ~/xm_entrepreneurship_system
java -jar springboot-0.0.1-SNAPSHOT.jar

# 按 Ctrl+A 然后按 D 退出screen（应用继续运行）

# 重新连接到screen
screen -r backend

# 查看所有screen会话
screen -ls

# 结束screen会话
screen -X -S backend quit
```

---

## 三、Docker容器管理

### 常用命令

```bash
# 查看所有容器
docker ps -a

# 查看运行中的容器
docker ps

# 启动容器
docker start entrepreneurship_backend

# 停止容器
docker stop entrepreneurship_backend

# 重启容器
docker restart entrepreneurship_backend

# 删除容器
docker rm entrepreneurship_backend

# 查看容器日志
docker logs -f entrepreneurship_backend

# 查看最近100行日志
docker logs --tail 100 entrepreneurship_backend

# 进入容器
docker exec -it entrepreneurship_backend /bin/bash

# 查看容器资源使用
docker stats

# 查看容器详细信息
docker inspect entrepreneurship_backend
```

### Docker自动重启

容器已配置 `--restart always`，系统重启后会自动启动：
- `no`: 不自动重启（默认）
- `always`: 总是重启
- `on-failure`: 只在容器非正常退出时重启
- `unless-stopped`: 除非手动停止，否则总是重启

---

## 四、日志查看和调试

### 1. 后端日志

#### Docker方式
```bash
# 实时查看日志
docker logs -f entrepreneurship_backend

# 查看最近100行
docker logs --tail 100 entrepreneurship_backend

# 查看带时间戳的日志
docker logs -t entrepreneurship_backend

# 查看特定时间的日志
docker logs --since "2024-12-04T10:00:00" entrepreneurship_backend
```

#### nohup方式
```bash
# 实时查看
tail -f app.log

# 查看最近100行
tail -100 app.log

# 搜索错误
grep ERROR app.log

# 搜索特定内容
grep "关键词" app.log
```

### 2. 前端日志

```bash
# Nginx访问日志
docker exec entrepreneurship_frontend cat /var/log/nginx/access.log

# Nginx错误日志
docker exec entrepreneurship_frontend cat /var/log/nginx/error.log

# 实时查看
docker exec entrepreneurship_frontend tail -f /var/log/nginx/access.log
```

### 3. MySQL日志

```bash
# 查看MySQL日志
docker logs entrepreneurship_mysql

# 查看慢查询日志
docker exec entrepreneurship_mysql cat /var/log/mysql/slow.log
```

### 4. 远程调试（可选）

#### 启用Java远程调试
```bash
# 修改启动命令，添加调试参数
java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005 \
     -jar springboot-0.0.1-SNAPSHOT.jar
```

#### IDEA配置远程调试
1. Run → Edit Configurations
2. 添加 → Remote JVM Debug
3. 配置：
   - Host: collegetopics.cn
   - Port: 5005
4. 设置断点后，点击Debug按钮连接

---

## 五、常见问题

### Q1: 终端关闭后应用会停止吗？

**A**: 看情况：
- ✅ **Docker容器**：不会停止，后台持续运行
- ✅ **nohup命令**：不会停止
- ✅ **screen会话**：不会停止
- ❌ **直接运行**：会停止

### Q2: 如何更新应用？

```bash
# 方式1: Docker更新
docker stop entrepreneurship_backend
docker rm entrepreneurship_backend
# 上传新的jar包后重新构建
docker build -t entrepreneurship-backend ./springboot
docker run -d ... entrepreneurship-backend

# 方式2: 直接替换jar包
# 上传新的jar包
kill <旧进程PID>
nohup java -jar springboot-0.0.1-SNAPSHOT.jar > app.log 2>&1 &
```

### Q3: 如何备份数据库？

```bash
# 导出数据库
docker exec entrepreneurship_mysql mysqldump \
  -uroot -proot123456 entrepreneurship_system > backup-$(date +%Y%m%d).sql

# 恢复数据库
docker exec -i entrepreneurship_mysql mysql \
  -uroot -proot123456 entrepreneurship_system < backup-20241204.sql
```

### Q4: 端口被占用怎么办？

```bash
# 查看端口占用
sudo netstat -tulpn | grep :9090

# 或使用
sudo lsof -i :9090

# 杀死占用端口的进程
sudo kill -9 <PID>
```

### Q5: 如何查看应用是否正常运行？

```bash
# 检查后端
curl http://collegetopics.cn:9090/

# 检查MySQL连接
docker exec entrepreneurship_mysql mysql -uroot -proot123456 -e "SELECT 1"

# 检查前端
curl http://collegetopics.cn/
```

### Q6: 文件上传后在哪里？

```bash
# Docker方式（映射到主机）
ls ~/xm_entrepreneurship_system/files/

# 进入容器查看
docker exec -it entrepreneurship_backend ls /app/files/
```

---

## 六、推荐的工作流程

### 开发阶段
```
1. 本地IDEA启动后端（连接远程MySQL）
2. 本地VSCode/WebStorm启动前端（npm run dev）
3. 浏览器访问 localhost:5173 测试
4. 使用IDEA断点调试后端
5. 使用浏览器开发者工具调试前端
```

### 部署阶段
```
1. 本地打包（mvn package + npm run build）
2. 上传到服务器
3. 使用Docker启动所有服务
4. 配置域名和HTTPS
5. 设置自动备份
```

### 日常维护
```
1. 定期查看日志：docker logs
2. 监控容器状态：docker ps
3. 备份数据库：mysqldump
4. 更新应用：替换jar/镜像
```

---

## 七、一键脚本

### 一键启动开发环境（本地）
创建 `start-dev.sh`:
```bash
#!/bin/bash
echo "启动开发环境..."

# 启动后端（后台）
cd springboot
nohup mvn spring-boot:run > ../backend.log 2>&1 &
BACKEND_PID=$!
echo "后端PID: $BACKEND_PID"

# 等待后端启动
sleep 10

# 启动前端
cd ../vue
npm run dev
```

### 一键部署到服务器（服务器）
创建 `deploy-all.sh`:
```bash
#!/bin/bash
set -e

echo "=== 开始部署 ==="

# 构建镜像
echo "构建后端镜像..."
docker build -t entrepreneurship-backend ./springboot

echo "构建前端镜像..."
docker build -t entrepreneurship-frontend ./vue

# 停止旧容器
echo "停止旧容器..."
docker stop entrepreneurship_backend entrepreneurship_frontend 2>/dev/null || true
docker rm entrepreneurship_backend entrepreneurship_frontend 2>/dev/null || true

# 启动新容器
echo "启动MySQL..."
docker start entrepreneurship_mysql || docker run -d \
  --name entrepreneurship_mysql \
  --restart always \
  -e MYSQL_ROOT_PASSWORD=root123456 \
  -e MYSQL_DATABASE=entrepreneurship_system \
  -p 3306:3306 \
  -v mysql_data:/var/lib/mysql \
  mysql:8.0

sleep 10

echo "启动后端..."
docker run -d \
  --name entrepreneurship_backend \
  --restart always \
  -p 9090:9090 \
  -v $(pwd)/files:/app/files \
  --link entrepreneurship_mysql:mysql \
  entrepreneurship-backend

sleep 5

echo "启动前端..."
docker run -d \
  --name entrepreneurship_frontend \
  --restart always \
  -p 80:80 \
  --link entrepreneurship_backend:backend \
  entrepreneurship-frontend

echo "=== 部署完成 ==="
docker ps
```

---

## 🎯 总结

**推荐流程**：
1. ✅ **开发时**：本地IDEA+VSCode，连接远程MySQL
2. ✅ **测试通过后**：打包上传，Docker部署到服务器
3. ✅ **日常维护**：docker logs查看日志，screen会话临时调试

Docker容器会一直后台运行，不需要保持终端打开！ 🚀

