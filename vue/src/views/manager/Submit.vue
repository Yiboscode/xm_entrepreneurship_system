<template>
  <div>
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.userName" placeholder="请输入用户查询" style="width: 240px; margin-right: 10px"></el-input>
      <el-input v-model="data.taskTitle" placeholder="请输入任务名称查询" style="width: 240px"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="title" label="成果简介" show-overflow-tooltip></el-table-column>
        <el-table-column prop="detail" label="任务成果" show-overflow-tooltip>
...
const data = reactive({
  ...
  rules: {
    userId: [
      { required: true, message: '请输入用户%name%name|用户名称', trigger: 'blur' },
    ],
    taskId: [
      { required: true, message: '请输入任务%title%title|任务标题', trigger: 'blur' },
    ],
    teacherId: [
...
const load = () => {
  ...
  request.get('/submit/selectPage', {params}).then(res => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/submit/delete/batch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()  // 刷新表格数据
      } else {
