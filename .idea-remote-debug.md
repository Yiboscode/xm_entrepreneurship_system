...
Name: 启动远程服务
Program: ssh
Arguments: ubuntu@119.29.152.18 "cd ~/xm_entrepreneurship_system && docker compose up -d"
...
Name: 停止远程服务
Program: ssh
Arguments: ubuntu@119.29.152.18 "cd ~/xm_entrepreneurship_system && docker compose down"
Working directory: $ProjectFileDir$
```
...
Name: 查看远程日志
Program: ssh
Arguments: ubuntu@119.29.152.18 "cd ~/xm_entrepreneurship_system && docker compose logs --tail=100 backend"
Working directory: $ProjectFileDir$
```
...
2. 添加 → Shell Script
3. Script text: `ssh ubuntu@119.29.152.18 "cd ~/xm_entrepreneurship_system && docker compose up -d"`
...
```bash
# 监听本地文件变化并同步
fswatch -o ./springboot | xargs -n1 -I{} rsync -avz --exclude 'target' ./springboot ubuntu@119.29.152.18:~/xm_entrepreneurship_system/
```
...
# 上传公钥到服务器
ssh-copy-id ubuntu@119.29.152.18
```
